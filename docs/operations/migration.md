...
The destination installer supports:

```bash
bash scripts/install-vm.sh --migrate-url "<one-time-migration-download-url>"
```

The migration URL is generated by the source Administration workflow and should be treated as sensitive, temporary access to the migration package.
