<script lang="ts">
	...
	$: cleanSeries = series.filter((sample) => Number.isFinite(sample) && sample >= 0);
	$: effectiveMax = maxValue && maxValue > 0
		? maxValue
		: Math.max(1, ...cleanSeries) * 1.15;
	$: points = cleanSeries.map((sample, index) => {
		const x = (index / Math.max(1, cleanSeries.length - 1)) * chartWidth;
		const level = Math.max(0, Math.min(1, sample / effectiveMax));
		const y = chartHeight - level * chartHeight;
		return `${x.toFixed(2)},${y.toFixed(2)}`;
	});
	$: linePath = points.length >= 2 ? `M ${points.join(' L ')}` : '';
	$: areaPath = points.length >= 2 ? `M 0 ${chartHeight} L ${points.join(' L ')} L ${chartWidth} ${chartHeight} Z` : '';
	$: resourceClass = {
		neutral: {
...
<article class={`min-w-0 p-4 ${className}`.trim()} aria-label={`${label}: ${value}`}>
	...
	<div class="mt-2 flex items-center justify-between gap-3 text-[11px] text-gray-500 dark:text-gray-400">
		...
		<span class="shrink-0 font-mono">{cleanSeries.length >= 2 ? rangeLabel : 'collecting'}</span>
	</div>
</article>
