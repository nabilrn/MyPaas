<script lang="ts">
	...
	const dispatch = createEventDispatcher<{ deploy: void; restart: void; stop: void }>();
...
<section class="surface overflow-visible">
	<div class="flex flex-col gap-4 p-5 sm:flex-row sm:items-start sm:justify-between">
		...
		<div class="flex shrink-0 items-center gap-2">
			<ActionButton variant="primary" on:click={() => dispatch('deploy')} disabled={deployDisabled} loading={deployLoading} loadingLabel={deployLoadingLabel}><Rocket slot="icon" class="h-4 w-4" />Deploy</ActionButton>
