...
<article class={`grid h-full min-h-52 min-w-0 grid-cols-[minmax(0,1fr)_7rem] items-center gap-3 p-4 ${className}`.trim()} aria-label={`${label}: ${value}`}>
	<div class="min-w-0">
		...
		{:else}
			<p class="mt-3 text-sm font-medium text-gray-400 dark:text-gray-500">No sample</p>
		{/if}
	...
	>
		<svg viewBox="0 0 96 104" class="h-32 w-28" aria-hidden="true">
