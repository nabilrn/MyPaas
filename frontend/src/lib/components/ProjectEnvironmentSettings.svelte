...
<SectionPanel title="Environment variables" description="Encrypted at rest. Reveal only when you need to inspect a stored value." contentClass="p-0">
			...
			<div class="grid xl:grid-cols-[minmax(0,1fr)_18rem]">
				...
				<div class="border-t border-gray-100 p-4 dark:border-neutral-800 xl:border-l xl:border-t-0">
					<div class="grid grid-cols-2 divide-x divide-y divide-gray-100 dark:divide-neutral-800">
						<div class="p-3"><p class="metric-label">New</p><p class="metric-value mt-1 text-xl font-semibold text-gray-950 dark:text-white">{importCounts.newCount}</p></div>
						<div class="p-3"><p class="metric-label">Overwrite</p><p class="metric-value mt-1 text-xl font-semibold text-gray-950 dark:text-white">{importCounts.overwrite}</p></div>
						<div class="p-3"><p class="metric-label">Duplicate</p><p class="metric-value mt-1 text-xl font-semibold text-gray-950 dark:text-white">{importCounts.duplicate}</p></div>
						<div class="p-3"><p class="metric-label">Invalid</p><p class="metric-value mt-1 text-xl font-semibold text-gray-950 dark:text-white">{importCounts.invalid}</p></div>
					</div>
					...
					{#if hasDirty}<div class="alert-warning mt-4 py-2.5 text-sm">Save or discard existing drafts before importing.</div>{/if}
			...
			{#if importRows.length > 0}
				<div class="overflow-x-auto border-t border-gray-100 bg-white dark:border-neutral-800 dark:bg-neutral-950/50">
					<table class="data-table min-w-[38rem]">
						<thead><tr><th class="w-16">Line</th><th>Key</th><th class="w-32">Value</th><th class="w-40">Status</th></tr></thead>
						<tbody>
	...
	{#if loading}
		<div class="flex min-h-40 items-center justify-center p-4"><LoadingIndicator label="Loading environment variables" /></div>
	{:else if error}
	...
	{#if adding}
		<div class="grid gap-3 border-t border-gray-100 bg-gray-50/50 p-4 dark:border-neutral-800 dark:bg-neutral-900/40 lg:grid-cols-[14rem_minmax(0,1fr)_auto] lg:items-end">
			...
			<div class="flex gap-2">
				<ActionButton variant="primary" on:click={handleAdd} loading={savingNewVar} loadingLabel="Adding" disabled={!canAdd}><Plus slot="icon" class="h-4 w-4" />Add</ActionButton>
				<ActionButton variant="ghost" on:click={() => (adding = false)} disabled={savingNewVar}><X slot="icon" class="h-4 w-4" />Cancel</ActionButton>
			</div>
		</div>
	{/if}
</SectionPanel>
