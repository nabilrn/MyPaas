...
<div class="page-shell py-6">
	...
	>
		<svelte:fragment slot="actions">
			<IconButton label="Refresh audit logs" variant="brand" loading={loading} on:click={load}>
					...
					{#each visibleRows as row}
						<tr class="align-top hover:bg-gray-50/80 dark:hover:bg-gray-900/70">
							...
							<td class="px-5 py-4 text-right">
								<ActionButton variant="ghost" size="xs" on:click={() => toggle(row.id)} ariaLabel={`${expanded.has(row.id) ? 'Hide' : 'Show'} audit log details`}>
