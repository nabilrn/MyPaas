...
<div class="page-shell py-6">
	<div class="mb-5 flex flex-wrap items-center justify-between gap-3">
		<p class="max-w-3xl text-sm text-gray-500 dark:text-gray-400">Authenticated control-plane changes across projects, deployments, environment variables, and admin access.</p>
		<div class="flex flex-wrap items-center gap-2">
			...
			<ActionLink href="/admin/users" variant="secondary" size="sm">
				<User slot="icon" class="h-4 w-4" />
				User whitelist
			</ActionLink>
		</div>
	</div>
	...
	<TableShell
		title="Event stream"
		description="Review what changed, which resource was touched, and the response returned by the control plane."
		{loading}
		loadingRows={3}
		{error}
		empty={rows.length === 0}
				...
				{#each visibleRows as row}
					<tr class="align-top">
						<td>
							...
							<p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">{row.ipAddress ?? 'unknown ip'}</p>
						</td>
						<td>
							<p class="text-sm text-gray-700 dark:text-gray-300">{row.resourceType ?? '—'}</p>
							{#if row.resourceId}
								<p class="mt-0.5 max-w-64 truncate font-mono text-xs text-gray-400 dark:text-gray-500" title={row.resourceId}>{row.resourceId}</p>
							{/if}
						</td>
						<td>
							<span class={`inline-flex items-center gap-2 font-mono text-sm ${statusTextClass(row.metadata.status)}`}>
								<span class={`status-dot ${statusDotClass(row.metadata.status)}`}></span>
					...
					{#if expanded.has(row.id)}
						<tr class="!bg-gray-50/70 dark:!bg-neutral-900/50">
							<td colspan="5" class="!p-4">
								<div class="grid gap-4 lg:grid-cols-[15rem_minmax(0,1fr)]">
									<div class="space-y-3 text-sm text-gray-500 dark:text-gray-400">
										<div>
											<p class="text-xs font-medium text-gray-700 dark:text-gray-200">IP address</p>
											<p class="mt-1 font-mono text-xs">{row.ipAddress ?? 'unknown'}</p>
										</div>
										<div>
											<p class="text-xs font-medium text-gray-700 dark:text-gray-200">User agent</p>
											<p class="mt-1 line-clamp-4 break-words text-xs">{row.userAgent ?? 'unknown'}</p>
										</div>
									</div>
									<pre class="console-surface max-h-80 overflow-auto p-3">{JSON.stringify(row.metadata, null, 2)}</pre>
								</div>
							</td>
						</tr>
					{/if}
