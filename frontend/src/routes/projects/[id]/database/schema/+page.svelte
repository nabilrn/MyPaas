<script lang="ts">
	import { LocateFixed, Minus, Plus, RefreshCw, Search } from '@lucide/svelte';
	...
	import { api } from '$api';
	import { layoutERD, type ERDDensity, type ERDDirection, type ERDEdge, type ERDNode } from '$lib/dbstudio/erd';
	...
	let density: ERDDensity = 'comfortable';
	let colorMode: 'neutral' | 'group' = 'neutral';
	...
	const groupClasses = [
		...
		'border-violet-200 bg-violet-50/80 dark:border-violet-900 dark:bg-violet-950/25',
		'border-amber-200 bg-amber-50/80 dark:border-amber-900 dark:bg-amber-950/25',
		'border-rose-200 bg-rose-50/80 dark:border-rose-900 dark:bg-rose-950/25'
	];
	...
	function edgePath(edge: ERDEdge) {
		...
		if (direction === 'LR') {
			const fromX = fromCenterX >= toCenterX ? edge.from.x : edge.from.x + edge.from.width;
			const toX = fromCenterX >= toCenterX ? edge.to.x + edge.to.width : edge.to.x;
			const middleX = (fromX + toX) / 2;
			return `M ${fromX} ${fromCenterY} H ${middleX} V ${toCenterY} H ${toX}`;
		}
		const fromY = fromCenterY >= toCenterY ? edge.from.y : edge.from.y + edge.from.height;
		const toY = fromCenterY >= toCenterY ? edge.to.y + edge.to.height : edge.to.y;
		const middleY = (fromY + toY) / 2;
		return `M ${fromCenterX} ${fromY} V ${middleY} H ${toCenterX} V ${toY}`;
	}

	function edgeLabelPosition(edge: ERDEdge) {
		return {
			x: (edge.from.x + edge.from.width / 2 + edge.to.x + edge.to.width / 2) / 2,
			y: (edge.from.y + edge.from.height / 2 + edge.to.y + edge.to.height / 2) / 2
		};
	}
	...
	function nodeClasses(node: ERDNode) {
		...
		const tone = colorMode === 'group'
			? groupClasses[node.group % groupClasses.length]
			: 'border-gray-200 bg-white dark:border-neutral-800 dark:bg-neutral-950';
		return `${tone} ${selected ? 'ring-2 ring-gray-950 dark:ring-white' : 'hover:border-gray-400 dark:hover:border-gray-600'} ${muted ? 'opacity-25' : 'opacity-100'}`;
	}
	...
	function fitGraph() {
		...
		zoom = Math.min(1, Math.max(0.55, targetWidth / Math.max(graph.width, 1)));
	}

	function changeZoom(delta: number) {
		zoom = Math.min(1.4, Math.max(0.5, Number((zoom + delta).toFixed(2))));
	}
	...
	<div class="space-y-4">
		<div class="surface flex flex-wrap items-end gap-3 p-3">
			...
			<div class="min-w-48 flex-1">
				...
				<div class="relative"><Search class="pointer-events-none absolute left-2.5 top-2.5 h-4 w-4 text-gray-400" /><input id="erd-search" class="field w-full pl-8 font-mono" placeholder="users, audit_logs…" bind:value={tableSearch} on:keydown={(event) => event.key === 'Enter' && focusFirstMatch()} /></div>
			</div>
			<div>
				<label class="field-label" for="erd-direction">Layout</label>
				<select id="erd-direction" class="field" bind:value={direction}><option value="LR">Left → right</option><option value="TB">Top → bottom</option></select>
			</div>
			<div>
				<label class="field-label" for="erd-density">Density</label>
				<select id="erd-density" class="field" bind:value={density}><option value="comfortable">Comfortable</option><option value="compact">Compact</option></select>
			</div>
			<div>
				<label class="field-label" for="erd-color">Color</label>
				<select id="erd-color" class="field" bind:value={colorMode}><option value="neutral">Neutral</option><option value="group">Relation groups</option></select>
			</div>
		...
		{:else}
			<div class="surface overflow-hidden">
				<div class="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 px-3 py-2 dark:border-neutral-800">
					<div class="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400"><span>{graph.nodes.length} tables</span><span>·</span><span>{graph.edges.length} relations</span><span>·</span><span>{graph.groups} groups</span></div>
					<div class="flex flex-wrap items-center gap-2">
						...
						<label class="flex items-center gap-1.5 text-xs text-gray-600 dark:text-gray-300"><input type="checkbox" bind:checked={showRelationLabels} /> FK labels</label>
				...
				<div class="max-h-[68vh] overflow-auto bg-gray-50/50 p-3 dark:bg-neutral-950/40">
					<div class="relative" style={`width:${graph.width * zoom}px;height:${graph.height * zoom}px;min-width:100%`}>
						<div class="absolute left-0 top-0 origin-top-left" style={`width:${graph.width}px;height:${graph.height}px;transform:scale(${zoom})`}>
							<svg class="pointer-events-none absolute inset-0" width={graph.width} height={graph.height} aria-hidden="true">
								...
								{#each graph.edges as edge (edge.id)}
									<path d={edgePath(edge)} fill="none" class="stroke-gray-300 dark:stroke-gray-700" stroke-width="1.5" marker-end="url(#erd-arrow)" />
									{#if showRelationLabels}{@const label = edgeLabelPosition(edge)}<text x={label.x} y={label.y - 5} text-anchor="middle" class="fill-gray-500 text-[10px] dark:fill-gray-400">{edge.foreignKey.column}</text>{/if}
								{/each}
							</svg>
							...
							{#each graph.nodes as node (node.id)}
								<button type="button" class={`absolute overflow-hidden rounded-lg border text-left shadow-sm transition ${nodeClasses(node)}`} style={`left:${node.x}px;top:${node.y}px;width:${node.width}px;height:${node.height}px`} on:click={() => (selectedTable = node.detail.name)}>
									<div class="border-b border-black/5 px-3 py-2 dark:border-white/10"><p class="truncate font-mono text-[10px] text-gray-400 dark:text-gray-500">{node.detail.schema}</p><p class="truncate text-sm font-semibold text-gray-950 dark:text-white">{node.detail.name}</p></div>
