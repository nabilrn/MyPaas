		...
		{#if status.connected}
			<div class="grid gap-5 lg:grid-cols-[18rem_minmax(0,1fr)]">
				...
				<TableShell
					...
					description={primaryColumns.length > 0 ? `Primary key: ${primaryColumns.map((column) => column.name).join(', ')}` : 'Edit/delete disabled because this table has no primary key.'}
					loading={loadingRows}
					error={rowsError}
					empty={!rows || rows.rows.length === 0}
					emptyTitle="No rows in this table."
					loadingRows={5}
