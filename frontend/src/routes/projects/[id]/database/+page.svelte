<script lang="ts">
	...
	async function loadRows(reset = true) {
		...
		if (!reset && (loadingRows || loadingMoreRows || !rows?.hasMore)) return;
		...
		<div class="grid h-[calc(100vh-15rem)] min-h-[30rem] min-w-0 overflow-hidden border-b border-[color:var(--workspace-divider)] bg-white dark:bg-neutral-950 lg:grid-cols-[16rem_minmax(0,1fr)]" data-db-studio-workspace>
			...
			<section class="flex min-h-0 min-w-0 flex-col">
				<div class="shrink-0 border-b border-[color:var(--workspace-divider)] px-4 py-2.5">
					...
					<p class="mt-0.5 truncate text-xs text-gray-500 dark:text-gray-400">{activeFilterCount > 0 ? `${activeFilterCount} active filter${activeFilterCount === 1 ? '' : 's'} · ${primaryColumns.length > 0 ? `Primary key: ${primaryColumns.map((column) => column.name).join(', ')}` : 'No primary key'}` : primaryColumns.length > 0 ? `Primary key: ${primaryColumns.map((column) => column.name).join(', ')}` : 'Edit/delete disabled because this table has no primary key.'}</p>
				</div>

				<div class="shrink-0 border-b border-[color:var(--workspace-divider)] px-3 py-2" data-db-row-toolbar>
					<div class="flex w-full flex-wrap items-center gap-2">
						<input value={rowSearch} class="field min-w-[14rem] flex-1" placeholder="Search rows" aria-label="Search database rows" on:input={(event) => (rowSearch = (event.currentTarget as HTMLInputElement).value)} on:keydown={(event) => { if (event.key === 'Enter') { event.preventDefault(); void applyRowFilters(); } }} />
					...
					{#if enumColumns.length > 0}
						<div class="mt-2 grid w-full gap-2 sm:grid-cols-2 xl:grid-cols-4">
							{#each enumColumns as column}
								<label class="block min-w-0"><span class="field-label truncate" title={column.name}>{column.name}</span><select class="field w-full" value={enumFilters[column.name] ?? ''} on:change={(event) => (enumFilters = { ...enumFilters, [column.name]: (event.currentTarget as HTMLSelectElement).value })}><option value="">Any value</option>{#each column.enumValues ?? [] as value}<option {value}>{value}</option>{/each}</select></label>
							{/each}
						</div>
					{/if}
				</div>
						...
						<table class="data-table min-w-full" data-db-row-table>
							...
							<tbody>
								{#each rows.rows as row, rowIndex}
									<tr>{#each columns as column}<td class="max-w-72 truncate font-mono text-xs text-gray-700 dark:text-gray-200" title={formatCell(row[column.name])}>{formatCell(row[column.name])}</td>{/each}<td class="text-right"><div class="flex justify-end gap-1"><IconButton label={`Edit database row ${rowIndex + 1}`} variant="ghost" on:click={() => openEdit(row)} disabled={!writeActive || primaryColumns.length === 0}><Pencil class="h-4 w-4" aria-hidden="true" /></IconButton><IconButton label={`Delete database row ${rowIndex + 1}`} variant="danger" on:click={() => openDelete(row)} disabled={!writeActive || primaryColumns.length === 0}><Trash2 class="h-4 w-4" aria-hidden="true" /></IconButton></div></td></tr>
								{/each}
							</tbody>
						</table>
					...
					<div class="grid gap-4 sm:grid-cols-2">
						{#each mutationMode === 'insert' ? insertColumns : writableColumns as column}
							<div class="min-w-0">
								...
								{:else}
									<input id={`db-value-${column.name}`} value={draftValues[column.name] ?? ''} class="field w-full font-mono" placeholder={mutationMode === 'insert' && columnHasDefault(column) && !touchedValues[column.name] ? defaultPlaceholder(column) : column.nullable ? 'nullable' : ''} on:input={(event) => setMutationValue(column.name, (event.currentTarget as HTMLInputElement).value)} />
								{/if}
								<div class="mt-1.5 flex min-h-5 flex-wrap items-center gap-x-3 gap-y-1 text-xs">
									{#if isTemporalColumn(column) && !databaseNowValues[column.name]}<button type="button" class="app-focus font-medium text-gray-500 hover:text-gray-950 dark:text-gray-400 dark:hover:text-white" on:click={() => useDatabaseNow(column.name)}>Use database time</button>{/if}
									{#if mutationMode === 'insert' && columnHasDefault(column) && (touchedValues[column.name] || databaseNowValues[column.name])}<button type="button" class="app-focus font-medium text-gray-500 hover:text-gray-950 dark:text-gray-400 dark:hover:text-white" on:click={() => useDatabaseDefault(column.name)}>Use database default</button>{/if}
